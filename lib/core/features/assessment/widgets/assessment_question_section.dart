...
class AssessmentQuestionSection extends StatelessWidget {
  ...
  const AssessmentQuestionSection({super.key, required this.controller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final currentQuestion =
          controller.currentAssessment.questions[controller
  ...
  List<Widget> _buildAnswersList(Question currentQuestion) {
    return currentQuestion.answers.map((answer) {
      return Padding(
        ...
        child: Obx(
          () => AssessmentAnswerTile(
            answer: answer,
            isSelected: controller.selectedAnswerId.value == answer.id,
